...
nameCheck.addEventListener('blur', function () {
  if (!validateName()) {
    nameCheckError.textContent = 'Vui lòng nhập tên hợp lệ từ 2 đến 50 ký tự và không chứa ký tự đặc biệt.';
...
emailCheck.addEventListener('blur', function () {
  if (!validateEmail()) {
    emailCheckError.textContent = 'Vui lòng nhập email hợp lệ.';
...
phoneCheck.addEventListener('blur', function () {
  if (!validateNumber()) {
    phoneCheckError.textContent = 'Vui lòng nhập số điện thoại hợp lệ.';
...
document.getElementById('contact').addEventListener('submit', function (event) {
  ...
  emailjs.send('service_65ungcd', 'template_suro9xv', {
    ...
    alert('Đã xảy ra lỗi khi gửi thông tin: ', error);
  });
});
